...
const AddImages = () => {
  ...
  const getImages = async () => {
    try {
      setLoader(true);
      const response = await axios.get("https://low-code-backend-vyps.vercel.app/api/get-images", {
        headers: {
  ...
  const selectCurrentImage = (image) => {
    ...
    const backgroundImage = `url('https://low-code-backend-vyps.vercel.app/api/image/${image?._id}')`;
    const imageData = {
          ...
          {uploadedImages.length > 0 &&
            uploadedImages?.map((el, i) => {
              return (
                ...
                >
                  <Image
                    src={`https://low-code-backend-vyps.vercel.app/api/${el._id}`}
                    height={50}
                    width={50}
